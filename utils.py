    ...
    with torch.no_grad():
        ...
        for batch in epoch_iterator_val:
            ...
            with torch.cuda.amp.autocast():
                val_outputs = sliding_window_inference(val_inputs, (config["input_size"], config["input_size"], config["input_size"]), 1, model)
...
def train(model, global_step, train_loader,val_loader,config, dice_val_best, global_step_best):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        if (global_step % config["eval_num"] == 0 and global_step != 0) or global_step == config["max_iterations"]:
            ...
            dice_val = validation(model, epoch_iterator_val, config, global_step)
            epoch_loss /= step
